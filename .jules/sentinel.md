...
## 2024-07-14 - Hardcoded Firebase API Key in JSON Config
**Vulnerability:** The Firebase API key was hardcoded directly in `firebase-applet-config.json`. Since this file is tracked by version control, the API key was exposed in the repository, presenting a critical security risk.
**Learning:** Hardcoding sensitive credentials, such as API keys, directly into configuration files that are checked into version control is an unsafe practice. Firebase initialization requires an API key, but it should be provided via environment variables.
**Prevention:** Remove hardcoded API keys from static configuration files. Merge the necessary static configuration with sensitive values loaded from environment variables (e.g., `process.env.NEXT_PUBLIC_FIREBASE_API_KEY`) at runtime during initialization.
