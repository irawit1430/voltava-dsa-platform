...
## 2025-02-14 - Missing Input Validation on AI API Endpoint\n**Vulnerability:** The `/api/gemini/generate` endpoint accepted arbitrary user input (`agentName`, `language`, `generateType`, `leadDetails`) without any length or type validation. This left the application vulnerable to processing excessively large or malformed payloads, which could lead to Denial of Service or prompt injection attacks on the third-party AI API.\n**Learning:** Relying on client-side constraints or implicit assumptions about the shape of incoming data is insufficient. Backend API routes must treat all incoming payload data as untrusted.\n**Prevention:** Implement strict native JavaScript type, schema, and length validation on all request variables immediately after parsing `req.json()`.
