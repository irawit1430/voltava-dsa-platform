...
## 2024-07-16 - Hardcoded API Key in Firebase Configuration
**Vulnerability:** The Firebase API key was hardcoded in `firebase-applet-config.json`, which is checked into version control. This risks exposing the API key publicly if the repository is public or to unauthorized users within a private repository, potentially leading to unauthorized usage or billing issues.
**Learning:** Hardcoding sensitive secrets like API keys directly into configuration files that are checked into source control is a critical vulnerability.
**Prevention:** Always use environment variables (e.g., `NEXT_PUBLIC_FIREBASE_API_KEY`) for sensitive information and merge them into the configuration during runtime or build time. Ensure configuration files committed to the repo only contain non-sensitive structural configuration.
