...
export async function POST(req: NextRequest) {
  try {
    ...
    // Security: Validate inputs to prevent DoS and injection
    if (typeof language !== 'string' || language.length > 50) {
      return NextResponse.json({ error: "Invalid language parameter" }, { status: 400 });
    }
    if (typeof generateType !== 'string' || !['script', 'whatsapp'].includes(generateType)) {
    ...
    if (!leadDetails || typeof leadDetails !== 'object' || JSON.stringify(leadDetails).length > 5000) {
      return NextResponse.json({ error: "Invalid or oversized leadDetails parameter" }, { status: 400 });
    }
